...
export default async (req, res) => {
  ...
  switch (method) {
    ...
    case 'POST':
      try {
        const sched = await Schedule.findOne(
            { $and: [{
            cancellationCode:req.body.cancellationCode,
    ...
    case 'PUT':
      try {
          var sub = "midWife_schedule._id"
          var subStat = "midWife_schedule.$.status"
          var resCalendar; 
